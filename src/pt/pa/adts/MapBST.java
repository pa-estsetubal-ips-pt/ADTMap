...
public class MapBST<K extends Comparable<K>, V> implements Map<K,V> {
    ...
    private BSTNode getLeftmostNode(BSTNode treeRoot) {
        ...
        //its the leftmost node within this tree
        BSTNode leftMost = treeRoot;
    ...
    private int size(BSTNode treeRoot) {
        ...
        int count=1;
        count+=size(treeRoot.left) + size(treeRoot.right); //
        return count;
    }
    ...
    public String toString() {
        ...
        return String.format("MapBST of size = %d and height = %d:", size(), height(this.root))
                + "\n"
                + sb.toString();
    }
    ...
    private int height(BSTNode treeRoot) {
        ...
        else return 1 + Math.max( height(treeRoot.left), height(treeRoot.right));
    }
